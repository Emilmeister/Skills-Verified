...
- **9 анализаторов** — 6 встроенных и 3 опциональных (Bandit, Semgrep, Grype)
- **Опциональный LLM-анализ** через любой OpenAI-совместимый API (OpenAI, Anthropic через прокси, Ollama, vLLM, LM Studio)
- **Trust Score A-F** с разбивкой по 6 категориям
- **CI/CD gate** — `--fail-on strict|standard|relaxed` для автоматической блокировки опасных навыков
...
| `--fail-on POLICY` | CI/CD gate: `strict`, `standard` или `relaxed` (exit 1 при нарушении) |
...
**Доступные имена анализаторов:** `pattern`, `cve`, `bandit`, `semgrep`, `guardrails`, `permissions`, `supply_chain`, `container`, `llm`
...
CLI → fetcher (clone/validate) → Pipeline
                        ...
                        ┌────────────┬───────┴────────┐
                        ▼            ▼                 ▼
                    Console      JSON file       --fail-on gate
                    (Rich)       (-o path)       (exit 0 or 1)
```
